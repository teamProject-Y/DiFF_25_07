                        ...
                        {articles.length > 0 ? (
                            articles.map(article => (
                                <tr
                                    ...
                                    onClick={() =>
                                        router.push(`/usr/article/detail?id=${article.id}`)
                                    }
                                >
                ...
                <div className="inline-flex justify-center text-xl mt-4">
                    {Array.from({ length: totalPage }, (_, idx) => idx + 1).map(i => (
                        <button
                            ...
                            onClick={() =>
                                router.push(
                                    `/usr/article/list?boardId=${boardId}&searchItem=${searchItem}&keyword=${keyword}&page=${i}`
                                )
                            }
                            className={`mx-1 w-8 ${
...
export async function getServerSideProps({ query, req }) {
    ...
    const listRes = await fetch(
        `${API_BASE}/usr/article/list?boardId=${boardId}&searchItem=${searchItem}&keyword=${encodeURIComponent(keyword)}&page=${page}`,
        {
            headers: { cookie: req.headers.cookie || '' },
            credentials: 'include',
        }
